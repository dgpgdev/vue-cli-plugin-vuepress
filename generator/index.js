module.exports = (api, options, rootOptions) => {
	...
	const dest = options.deploy ? (options.repotype === 'github' ? 'vuepress' : 'public') : 'vuepress'
	conf.dest = dest
	...
	api.onCreateComplete(() => {
		...
		if (options.repotype === 'firebase') {
			createFile('./.firebaserc', makeFirebaseRC(options.firebaseID))
			createFile('./firebase.json', makeFirebaseJson('./' + dest))
		}
		...
		if (options.externalComponent) {
			createFile(
				rootDoc + '/.vuepress/enhanceApp.js',
				makeEnhance(rootDoc + '/.vuepress', options.folderComponent),
			)
		}
	})
}
